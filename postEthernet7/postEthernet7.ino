...
void loop()
{
    ...
    eth_uploadData_job();//write to server
    while (client.connected())//server answer: end of rx = '?'
    {
        if (client.available())
        {
              char rx= client.read();
...
uint8_t eth_uploadData_job(void)
{
  ...
  if (eth_data2send_length > 0)
  {
    if (client.connect(server, 80))
    {
        ...
        strcpy_P(filePath, (const char *) &eth_filePath);
        
        client.print(F("POST ");client.print(eth_filePath);client.println(F(" HTTP/1.1"));
