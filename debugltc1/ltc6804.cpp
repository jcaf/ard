...
#define LTC6804_SPI_SPEED 500E3

    #define ATMEGA_SPI_SS 53
void ltc6804_SPI_access(void)
{
    SPI.endTransaction();
...
uint8_t ltc6804_job(void)
{
    ...
    if (sm0 == 1)
    {
        if ( (millis() - last_millis) > 250)//210 standard
        {
            pec = ltc_rdaux_groupAB_allIC(gpio_matrix);
    ...
    if (sm0 == 2)
    {
        ...
        //delay(300);//ni un delay despues de este comando! 
    }
...
static void ltc_job_print2user(void)
{
    for (uint8_t num_ic = 0; num_ic < NUM_IC; num_ic++)
    {
        if (ltc_connected[num_ic] == LTC_CONNECTED)
        {
            ...
            Serial.print(num_ic);
            Serial.print(F(" ["));
            Serial.print(select_2_12V[num_ic]);
            Serial.println(F("V] "));
            //__
...
static uint8_t ltc_get_connectedStatus(void)
{
    ...
    uint16_t gpio_matrix1[NUM_IC][NUM_TOTAL_REGAUX];
    uint8_t finish=0;
    ...
    if (sm0 == 1)
    {
        if ( (millis() - last_millis) > 100)//2)//2ms
        {
            ltc_rdaux_groupAB_allIC(gpio_matrix1);
            ltc_set_connectedStatus(gpio_matrix1);
...
/////////////////////////////////////////////////////////////////////////////////////
static void ltc_set_connectedStatus(uint16_t (*pgpio_matrix)[NUM_TOTAL_REGAUX])//
{
    ...
    for (uint8_t num_ic = 0; num_ic < NUM_IC; num_ic++)
    {
        ...
        {
            //if (pgpio_matrix[num_ic][num_regaux] == 0xFFFF)
            if (pgpio_matrix[num_ic][num_regaux] > 65000)
                count_presence++;
        }
...
static void parse_gpio(uint16_t (*gpio_matrix)[NUM_TOTAL_REGAUX])//
{
    for (uint8_t num_ic = 0; num_ic < NUM_IC; num_ic++)
    {
        if (ltc_connected[num_ic] == LTC_CONNECTED)
        {
            ...
            //4.- get stack current
            stack_currentF[num_ic] = get_stack_current(gpio_matrix[num_ic][AUX_GPIO_CURRENTSENSE] / LTC_ADC_RESOL);
        }
...
static uint8_t read_cellvoltage(void)
{
    ...
    if (ltc_connected[num_ic] == LTC_CONNECTED)
    {
        if (sm0 == 0)
        {
            if (select_2_12V[num_ic] == 2)
            {
                ltc_write_cfg_address(num_ic, uv_2v+FACTCORR_2V, ov_2v+FACTCORR_2V, 0x0000);
        ...
        if (sm0 == 1)
        {
            ...
            //enters either the REFUP or MEASURE state.
            ...
            if ( (millis() - last_millis) > 250)
            {
                ...
                Serial.print(select_2_12V[num_ic]);
                Serial.println(F("V] "));
                Serial.println(F("read_cellvoltage (LTC6804_RDCV_BYGROUP_address) 1-12:"));
                Serial.println(F(""));
                for (uint8_t num_cell = 0; num_cell < LTC_NUM_CELLS; num_cell++)
                {
                    Serial.print(F("celda"));
                    ...
                    Serial.print(F(": "));
                    Serial.println(data_read[num_cell]);
                }
                Serial.println(F(""));
#endif
...
static void cellvoltage_print2user(uint8_t num_ic)
{
    ...
    for (uint8_t num_cell = 0; num_cell < NUM_CELL_TOP; num_cell++)
    {
        ...
        Serial.print(num_cell + 1);
        Serial.print(F(": "));
        Serial.print( vcellF[num_ic][num_cell], NUM_DECIMALS);
        Serial.println(F(" V"));
    }
    Serial.println(F(""));

    Serial.print(F("stack total voltage: "));
    Serial.print(stack_voltage_ifcF[num_ic], NUM_DECIMALS);
    Serial.println(F(" V"));
    Serial.println(F(""));
}
...
static void status_print2user(uint8_t num_ic)
{
    ...
    Serial.print(F("internal Sum Of Cell (SOC): "));
    Serial.print(internal_socF[num_ic], NUM_DECIMALS_GRL);
    Serial.println(F(" V"));
    Serial.print(F("internal die temp.: "));
    Serial.print(internal_die_tempF[num_ic], NUM_DECIMALS_GRL);
    Serial.println(F(" C"));
    Serial.print(F("internal analog VA (4.5V-5.5V): "));
    Serial.print(internal_analog_powerF[num_ic], NUM_DECIMALS_GRL);
    Serial.println(F(" V"));
    Serial.print(F("internal digital VD (2.7V-3.6V): "));
    Serial.print(internal_digital_powerF[num_ic], NUM_DECIMALS_GRL);
    Serial.println(F(" V"));
    ...
    for (uint8_t num_cell = 0; num_cell < NUM_CELL_TOP; num_cell++)
    {
        ...
        Serial.print(num_cell + 1);
        Serial.print(F(" UV: "));
        Serial.println(uv_flag[num_ic][num_cell] );
    }
    ...
    for (uint8_t num_cell = 0; num_cell < NUM_CELL_TOP; num_cell++)
    {
        ...
        Serial.print(num_cell + 1);
        Serial.print(F(" OV: "));
        Serial.println(ov_flag[num_ic][num_cell] );
    }
...
static void aux_print2user(uint8_t num_ic)
{
    ...
    Serial.print(F("GPIO1 - current sense: "));
    Serial.print(vgpioF[num_ic][AUX_GPIO_CURRENTSENSE], NUM_DECIMALS_GRL + 2);
    Serial.println(F(" V"));
    Serial.print(F("GPIO2 - select 12V/2V: "));
    Serial.print(vgpioF[num_ic][AUX_GPIO_SELECT_2_12V], NUM_DECIMALS_GRL);
    Serial.println(F(" V"));
    Serial.print(F("GPIO3 - stack-union 24 cell: "));
    Serial.print(vgpioF[num_ic][AUX_STACK_UNION], NUM_DECIMALS_GRL);
    Serial.println(F(" V"));
    Serial.print(F("GPIO4 - NTC10K / VREF2 buff opamp: "));
    Serial.print(vgpioF[num_ic][AUX_GPIO_NTC10K], NUM_DECIMALS_GRL);
    Serial.println(F(" V"));
    Serial.print(F("GPIO5 - LTC6655CHMS-3.3V 0.05% 5ppm/C: "));
    Serial.print(vgpioF[num_ic][AUX_GPIO_VREF_EXT], NUM_DECIMALS_GRL);
    Serial.println(F(" V"));
    Serial.print(F("VREF2(2.985V-3.015V): "));
    Serial.print(vgpioF[num_ic][AUX_LTC_VREF2], NUM_DECIMALS_GRL);
    Serial.println(F(" V"));
...
static void monitor_stack_current_print2user(uint8_t num_ic)
{
    ...
    Serial.print(F("stack DC-current : "));
    Serial.print(stack_currentF[num_ic], NUM_DECIMALS_GRL);
    Serial.println(F(" A"));

    Serial.print(F("over permissible limit (0=OK 1=BAD): "));
    Serial.println(stack_overCurrent[num_ic]);
    Serial.println(F(""));
}
/////////////////////////////////////////////////////////////////////////////////////
...
static void monitor_stack_temperature(void)
{
    for (uint8_t num_ic = 0; num_ic < NUM_IC; num_ic++)
    {
        if (ltc_connected[num_ic])
        {
            if (stack_temperatureF[num_ic] > 30.0 )
            {
...
void monitor_stack_uv(void)
{
    ...
    for (uint8_t num_ic = 0; num_ic < NUM_IC; num_ic++)
    {
        if (ltc_connected[num_ic])
        {
            ...
            for (uint8_t num_cell = 0; num_cell < NUM_CELL_TOP; num_cell++)
            {
                if (uv_flag[num_ic][num_cell] )
                    count_uv++;
            }

            if (count_uv == NUM_CELL_TOP)
            {
...
static void monitor_stack_temperature_print2user(uint8_t num_ic)
{
    ...
    Serial.print(F("stack temperature: "));
    Serial.print(stack_temperatureF[num_ic]);
    Serial.println(F(" C"));
    Serial.println(F(""));
}
/////////////////////////////////////////////////////////////////////////////////////
...
static void stack_union_print2user(uint8_t num_ic)
{
    ...
    if (num_ic % 2 != 0)
    {
        Serial.print(stack_union[num_ic]);
        Serial.print(F(": "));
