          ...
          for (uint8_t y=0; y<LTC_NUM_CELLS; y++ )
            {
              ...
              Serial.print(vcellF[i][y] + String(" "));
            }
...
v2server[] PROGMEM=
{
    ...
    {idx_stack_voltage_ifcF, "d4",(float *)stack_voltage_ifcF,{NUM_IC,0}, sizeof(stack_voltage_ifcF[0]),""},
    {idx_stack_currentF, "d5",(float *)stack_currentF,{NUM_IC,0}, sizeof(stack_currentF[0]),""},
    {idx_stack_temperatureF, "d6",(float *)stack_temperatureF,{NUM_IC,0}, sizeof(stack_temperatureF[0]),""},
    {idx_select_2_12V, "d7",(uint8_t *)select_2_12V,{NUM_IC,0}, sizeof(select_2_12V[0]),""},
    {idx_stack_union, "d8",(uint8_t *)stack_union,{NUM_IC,0}, sizeof(stack_union[0]),""},
    {idx_vcellF, "d9",(float *)vcellF,{NUM_IC,LTC_NUM_CELLS}, sizeof(vcellF[0][0]),""},
      ...
      else
      {
        ...
        for (uint8_t f=0; f<fil; f++)
        {
            for (uint8_t c=0; c<col ; c++)
            {
                ...
                {
                    *((float *)pointer) = get_float_random(10,48);
                }
                ...
                {
                    if (select_2_12V[f] == 2)
                        *((float *)pointer) = get_float_random(1,3);
                    else if (select_2_12V[f] == 12)
                        *((float *)pointer) = get_float_random(11,13);
