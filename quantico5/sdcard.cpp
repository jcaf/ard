...
{
    char filepath_buff[SDCARD_PATH_DIR_MAX_LENGTH + SDCARD_NAME_FILE_MAX_LENGTH];
    strcpy_P(filepath_buff, filepath);

    char setting_name_toread_buff[SDCARD_SETTING_NAME_MAX_LENGTH];
    ...
    if (sdcard.begin)
    {
        ...
        if (myFile)
        {
            while (myFile.available())
            {
                ...
                {
                    if (String(setting_name_toread_buff) == setting_name)
                    {
                        //*setting_value_read = setting_value;
                        setting_value.toCharArray(setting_value_read, setting_value.length()+1);
                        value = 1;
                        break;
                    }
                }
