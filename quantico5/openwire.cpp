...
uint8_t openwire_2v(uint8_t num_ic)
{
    ...
    if (sm0 == 1)
    {
        ltc_write_cfg_address(num_ic, uv_2v, ov_2v, 0x0000 );//uv ov irrelevant
    ...
    if (sm0 > 2)
    {
        ...
        {
            if (cell_DIFF[num_cell] < -0.4)//< -400mV
            {
                openwire_flag[num_ic][num_cell-1] = 1;
            ...
            {
                openwire_flag[num_ic][num_cell-1] = 0;
            }
        }
