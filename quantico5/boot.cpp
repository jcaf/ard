...
} mainvar[]=
{
    ...
    {"Server1IP","192.168.0.35"},
    {"Host","githubprototypes.battery.com"},
    {"Directory","/drafts/jsondecode_nuevo.php"},
...
uint8_t save_corfact_vars(String *setting_name, String *setting_value)
{
    ...
    for (uint8_t i=0; i<CORFACT_VARS_LENGTH; i++ )
    {
        ...
        if (String(buff) == *setting_name)
        {
            ...
            *corfact_vars[i].var= strtod(buff, (char**)0);
            //sysPrint_serialPrintln("var updated :"+ String(*corfact_vars[i].var,4));
            cod_ret = 1;
            break;
        }
    }
...
uint8_t save_setacq_vars(String *setting_name, String *setting_value)
{
    ...
    for (uint8_t i=0; i<SETACQ_VARS_LENGTH; i++ )
    {
        ...
        {
            setting_value->toCharArray(buff, setting_value->length()+1);
            *setacq_vars[i].var= strtod(buff, (char**)0);
            //sysPrint_serialPrintln("var updated :"+ String(*setacq_vars[i].var,4));
            cod_ret = 1;
            break;
        }
    }
