...
float DC_SHUNT_RESISTANCE = (shuntVoltFullScale/shuntRatedCurrent);//5E-5
...
float get_stack_current(uint16_t output_xgain)// dc mv *  gain
{
    ...
    float current = (voutput_xgain * shuntRatedCurrent)/ (shuntVoltFullScale * CSA_GAIN);
...
float Pa_POWER_AT_TEMP_AMB ( (2.0/3.0)*shuntRatedCurrent*shuntVoltFullScale );//Pa = 0.667 x shunt’s rated power at 25°C ambient
...
uint8_t current_over_permissible_limit(float current, float temperature)
{
    ...
    {
        if ( current > (DERATING_FACTOR_CONTINUOS_AT_TEMP_AMB*shuntRatedCurrent ) )
            ov_flag = 1;
        else
            ov_flag = 0;
    }
