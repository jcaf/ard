...
#define KB_KEY_PINLEVEL_PRESSED 1
#define KB_KEY_PINLEVEL_RELEASED 0

//set software
#define KB_KEY_STATE_PRESSED 1
#define KB_KEY_STATE_RELEASED 0

//Re-mapeo de switch - MAINBOARD
#define KB_PIN_MENU 2
#define KB_PIN_UP 13//21//21-> ARD_PWM_13
#define KB_PIN_DOWN 12//20//20-> ARD_PWM_12
...
static inline uint8_t keyRead_pin_menu(void)
{
    return digitalRead(KB_PIN_MENU);
}
static inline uint8_t keyRead_pin_up(void)
{
    return digitalRead(KB_PIN_UP);
}
static inline uint8_t keyRead_pin_down(void)
{
    return digitalRead(KB_PIN_DOWN);
}
static inline uint8_t keyRead_pin_enter(void)
{
    return digitalRead(KB_PIN_ENTER);
}
static inline uint8_t keyRead_pin_escape(void)
{
    return digitalRead(KB_PIN_ESCAPE);
}


volatile _key key[KB_NUM_KEYS];
...
void key_initialization(void)
{
    ...
    key[0].keyRead = keyRead_pin_menu;
    key[1].keyRead = keyRead_pin_up;//REPEAT
    key[2].keyRead = keyRead_pin_down;//REPEAT
    key[3].keyRead = keyRead_pin_enter;//REPEAT but now
    key[4].keyRead = keyRead_pin_escape;
