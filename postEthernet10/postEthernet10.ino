...
#include <SPI.h>
byte eth_mac[] = { 0x90, 0xA2, 0xDA, 0x0D, 0x4E, 0xD7 };
byte eth_server[] = { 192,168,1,34 };
const char eth_filePath[] PROGMEM ="/jsondecode.php";
...
uint8_t ltc_connected[NUM_IC]= {1,1,1,1,1,1,1,1};//all stacks connected
...
//////////////
    eth_uploadData_job();       //write to eth_server
...
//                Serial.println("?");
    ...
    client.stop();
    /*
    while (client.connected())  //eth_server answer: end of rx = '?'
    {
        if (client.available())
        {
            char rx= client.read();
    ...
    */
    //delay(120000);//2 min
    //delay(10000);//2 min (cat /proc/sys/net/ipv4/tcp_fin_timeout = 60s)
...
{
    {idx_vcellF1, "d1",(float *)&vcellF[0][0],{2,LTC_NUM_CELLS}, sizeof(vcellF[0][0]),""},
    {idx_vcellF2, "d2",(float *)&vcellF[2][0],{2,LTC_NUM_CELLS}, sizeof(vcellF[2][0]),""},
    {idx_vcellF3, "d3",(float *)&vcellF[4][0],{2,LTC_NUM_CELLS}, sizeof(vcellF[4][0]),""},
    /*
    ...
    {idx_fuelsensor_model, "c2",(uint8_t *)&fuelsensor.fsEE.model,{0,0}, sizeof(uint8_t),""},
    {idx_fuelsensor_outputtype_type, "c3",(uint8_t*)&fuelsensor.fsEE.outputType.type,{0,0}, sizeof(uint8_t),""},
    {idx_fuelsensor_outputtype_length_units, "c4",(uint8_t *)&fuelsensor.fsEE.outputType.length.units,{0,0}, sizeof(uint8_t),""},
    {idx_fuelsensor_outputtype_volume_units, "c5",(uint8_t *)&fuelsensor.fsEE.outputType.volume.units,{0,0}, sizeof(uint8_t),""},
    {idx_fuelsensor_tank_type, "c6",(uint8_t *)&fuelsensor.fsEE.tank.type,{0,0}, sizeof(uint8_t),""},
    {idx_fuelsensor_tank_rectangular_area_length, "c7",(float *)&fuelsensor.fsEE.tank.rectangular.area.length,{0,0}, sizeof(float),""},
    {idx_fuelsensor_tank_rectangular_area_width, "c8",(float *)&fuelsensor.fsEE.tank.rectangular.area.width,{0,0}, sizeof(float),""},
    {idx_fuelsensor_tank_irregular_spline_node_units_length_units, "c9",(uint8_t *)&fuelsensor.fsEE.tank.irregular.spline.node_units.length.units,{0,0}, sizeof(uint8_t),""},
    {idx_fuelsensor_tank_irregular_spline_node_units_volume_units, "c10",(uint8_t *)&fuelsensor.fsEE.tank.irregular.spline.node_units.volume.units,{0,0}, sizeof(uint8_t),""},
    {idx_fuelsensor_calib_tank_innertank, "c11",(float *)&fuelsensor.fsEE.calib.tank.innertank,{0,0}, sizeof(float),""},
    {idx_fuelsensor_calib_tank_zero2full, "c12",(float *)&fuelsensor.fsEE.calib.tank.zero2full,{0,0}, sizeof(float),""},
    {idx_fuelsensor_calib_tank_full2upper, "c13",(float *)&fuelsensor.fsEE.calib.tank.full2upper,{0,0}, sizeof(float),""},
    ...
    {idx_sdcardVar_shuntMultiplier, "c24",(float *)&shuntMultiplier,{0,0}, sizeof(float),""  },
    {idx_sdcardVar_uv_2v, "c25",(float *)&uv_2v,{0,0}, sizeof(float),""  },
    {idx_sdcardVar_ov_2v, "c26",(float *)&ov_2v,{0,0}, sizeof(float),""  },
    {idx_sdcardVar_uv_12v_S3C, "c27",(float *)&uv_12v_S3C,{0,0}, sizeof(float),""  },
    {idx_sdcardVar_ov_12v_S3C, "c28",(float *)&ov_12v_S3C,{0,0}, sizeof(float),""  },
    */
};
...
void init_data(void)
{
    ...
    fuelsensor.fsEE.model = 1;
    fuelsensor.fsEE.outputType.type = (FS_OUTPUTTYPE)2;
    fuelsensor.fsEE.outputType.length.units = 3;
    fuelsensor.fsEE.outputType.volume.units =4;
    fuelsensor.fsEE.tank.type = 5;
    fuelsensor.fsEE.tank.rectangular.area.length = 42.123;
    fuelsensor.fsEE.tank.rectangular.area.width  = 12.345;
    fuelsensor.fsEE.tank.irregular.spline.node_units.length.units = 6;
    fuelsensor.fsEE.tank.irregular.spline.node_units.volume.units = 7;
    fuelsensor.fsEE.calib.tank.innertank = 12.345;
    fuelsensor.fsEE.calib.tank.zero2full = 45.678;
    fuelsensor.fsEE.calib.tank.full2upper = 90.123;
...
*/
    eth_datasend_setFreq(ETH_DATASEND_SETFREQ_ALWAYS);
    //eth_datasend_setFreq(ETH_DATASEND_SETFREQ_ONCE_AND_POP, 15);
    
    //eth_datasend_setFreq(ETH_DATASEND_SETFREQ_ONCE_AND_POP, 28+10+5);
}
...
uint8_t eth_uploadData_job(void)
{
    ...
    if (eth_data2send_length > 0)
    {
        if (client.connect(eth_server, 80))
        {
            ...
            client.print(F("POST ")); client.print(filePath); client.println(F(" HTTP/1.1"));
...
void random_data(void)
{
    ...
    for (uint8_t v=0; v<NUM_VECTOR2SERVER; v++)
    {
        //1.- Set values
        idx = pgm_read_byte_near((const uint8_t *) &v2eth_server[v].idx);
        strcpy_P(name, (const char *) &v2eth_server[v].name);
        dimx    = pgm_read_word_near((const int16_t *) &v2eth_server[v].dim[0]);
        dimy    = pgm_read_word_near((const int16_t *) &v2eth_server[v].dim[1]);
        element_size = pgm_read_byte_near((const uint8_t *) &v2eth_server[v].element_size);

        if (dimx == LENGTH_TO_RESOLVE )
        {
            strcpy_P(aux0, (const char *) &v2eth_server[v].aux0);
            mainvar_getvalue(aux0).toCharArray(aux0, 30);
        ...
        {
            pointer = (void *) pgm_read_word(&(v2eth_server[v].pointer));
        }
